...
router.post('/admin/create', verifyAdmin, async (req, res) => {
    const { 
        name, email, phoneNo, service, amount, merchantType, brandId, 
        companyName, leadId, clientId 
    } = req.body;
...
router.get('/public/:linkId', async (req, res) => {
    ...
    try {
        ...
        // Frontend ko merchant ID bhej dein taki woh sahi client ID se PayPal SDK load kare
        const client_id = linkDetails.merchantType === 'paypal1' 
        ...
        return res.status(200).json({
            success: true,
            data: linkDetails,
            paypalClientId: client_id // Frontend ke liye zaroori
        });
...
router.post('/public/:linkId/create-order', async (req, res) => {
    ...
    try {
        ...
        const orderData = {
            ...
            application_context: {
                return_url: `${FRONTEND_BASE_URL}/payment/${linkId}?status=success`, 
                cancel_url: `${FRONTEND_BASE_URL}/payment/${linkId}?status=cancelled`,
                user_action: 'PAY_NOW',
            }
        };
        ...
        if (!response.ok) {
            ...
            return res.status(response.status).json({ message: data.message || "Failed to create PayPal order" });
        }
        ...
        return res.status(500).json({ message: error.message || "Internal server error during order creation" });
    }
});
...
router.post('/public/:linkId/charge', async (req, res) => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return res.status(response.status).json({ message: data.message || "Payment capture failed" });
        }
        
        // Zaroori Step: Database Update (Only if PayPal reports 'COMPLETED')
        ...
        return res.status(500).json({ message: error.message || "Internal server error during payment capture" });
    }
});
