...
const TimeIn = asyncHandler(async (req, res) => {
  ...
  // Midnight logic: 12AM–6AM → assign to previous date
  const shiftDate =
    currentHour >= 0 && currentHour < 6
      ? nowPKT.clone().subtract(1, "days").format("YYYY-MM-DD")
      : nowPKT.format("YYYY-MM-DD");
  ...
  const attendance = await Attendance.create({
    ...
    status: getAttendanceStatus(nowPKT, employee.shiftStart),
    lastActivity: nowPKT.toDate(),
  });
...
const updateOrMarkAttendance = asyncHandler(async (req, res) => {
  const { employeeId, shiftDate, timeIn, timeOut } = req.body;

  if (!employeeId || !shiftDate)
    throw new ApiError(400, "Employee ID and Shift Date are required");

  const employee = await Employee.findById(employeeId);
  if (!employee) throw new ApiError(404, "Employee not found");

  const parsePKT = (timeStr) =>
    moment.tz(`${shiftDate} ${timeStr}`, "YYYY-MM-DD hh:mm A", "Asia/Karachi");

  let updateData = {};

  if (timeIn) {
    const timeInMoment = parsePKT(timeIn);
    const status = getAttendanceStatus(timeInMoment, employee.shiftStart);
    updateData.timeIn = adjustForMidnight(timeInMoment).toISOString();
    updateData.status = status;
  }

  if (timeOut) {
    const timeOutMoment = parsePKT(timeOut);
    console.log(adjustForMidnight(timeOutMoment).toISOString(), "timeOutMoment");
    updateData.timeOut = adjustForMidnight(timeOutMoment).toISOString();

    // timeOut ke waqt final status recalculate karo
    // Pehle existing timeIn fetch karo agar is request mein timeIn nahi aaya
    let timeInMoment;
    if (timeIn) {
      timeInMoment = parsePKT(timeIn);
    } else {
      const existing = await Attendance.findOne({ employeeId, shiftDate });
      if (!existing?.timeIn)
        throw new ApiError(
          400,
          "TimeIn record not found. Cannot calculate status.",
        );
      timeInMoment = moment(existing.timeIn).tz(TZ);
    }

    const baseStatus = getAttendanceStatus(timeInMoment, employee.shiftStart);
    updateData.status = resolveFinalStatus(
      adjustForMidnight(timeInMoment),
      adjustForMidnight(timeOutMoment),
      baseStatus,
    );
  }
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        attendance,
        `Attendance updated successfully with status: ${updateData.status || "unchanged"}`,
      ),
    );
});

const TimeOut = asyncHandler(async (req, res) => {
  const nowPKT = moment().tz(TZ);

  // Pehle existing attendance fetch karo (timeIn chahiye status ke liye)
 ...
 const   employee = await Employee.findByIdAndUpdate(attendance.employeeId);
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, updated, "Attendance Timed Out"));
});
...
const getAllEmployeeAttendance = asyncHandler(async (req, res) => {
  ...
  const isSingleDay =
    (startDate && endDate && startDate === endDate) || (!startDate && !month);
  ...
  const stats = {
    ...
    present: records.filter((r) => r.shiftDate === (targetDate || startDate))
      .length,
...
//   const employee = await Employee.findById(employeeId); // req.user._id ki jagah employeeId use karein jo body se arha h
//   if (!employee) throw new ApiError(404, "Employee not found");

//   let updateData = {};

//   // 1. Time string ko Karachi timezone ke object mein convert karein
//   if (timeIn) {
//     const timeInMoment = moment.tz(
//       `${shiftDate} ${timeIn}`,
//       "YYYY-MM-DD hh:mm A",
//       "Asia/Karachi",
//     );
//     updateData.timeIn = timeInMoment.toISOString();
//     console.log(timeInMoment, "timeInMoment");
//     console.log(updateData.timeIn, "updateData.timeIn");

//     // --- Late & Half Day Logic based on PROVIDED timeIn ---
//     let attendanceStatus = "present";

//     // Employee ki shift start time nikalein
//     const shiftTimeStr = employee?.shiftStart || "20:30";
//     const [sHour, sMin] = shiftTimeStr.split(":").map(Number);

//     // Shift Start ka moment banayein usi shiftDate par
//     const shiftStartOnDate = moment.tz(shiftDate, "Asia/Karachi").set({
//       hour: sHour,
//       minute: sMin,
//       second: 0,
//     });

//     const graceTime = shiftStartOnDate.clone().add(30, "minutes");
//     const hourOfTimeIn = timeInMoment.hour();

//     console.log(hourOfTimeIn, "hourOfTimeIn");
//     // Logic: Agar subah 0 se 6 ke darmiyan login hai
//     if (hourOfTimeIn >= 0 && hourOfTimeIn < 6) {
//       updateData.timeIn = timeInMoment.clone().add(1, "days").toISOString();
//       attendanceStatus = "half-day";
//     }
//     // Agar shift start ke 30 mins baad login hai
//     else if (timeInMoment.isAfter(graceTime)) {
//       attendanceStatus = "late";
//     }
...
//   if (timeOut) {
//     const timeOutMoment = moment.tz(
//       `${shiftDate} ${timeOut}`,
//       "YYYY-MM-DD hh:mm A",
//       "Asia/Karachi",
//     );
//     updateData.timeOut = timeOutMoment.toISOString();

//     const hourOfTimeOut = timeOutMoment.hour();
//     console.log(hourOfTimeOut, "hourOfTimeOut");
//     if (hourOfTimeOut >= 0 && hourOfTimeOut < 6) {
//       updateData.timeOut = timeOutMoment.clone().add(1, "days").toISOString();
//     }
//   }

//   // 2. Update ya Create logic
//   const attendance = await Attendance.findOneAndUpdate(
//     { employeeId, shiftDate },
//     { $set: updateData },
//     { new: true, upsert: true },
...
//         200,
//         attendance,
//         "Attendance updated successfully with status: " +
//           (updateData.status || "unchanged"),
//       ),
//     );
// });
